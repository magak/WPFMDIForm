DROP PROCEDURE [dbo].[sp_CalcHVSForPeriod]
GO

CREATE PROCEDURE [dbo].[sp_CalcHVSForPeriod](@period int)	
AS
BEGIN
	SET NOCOUNT ON;
	...
	select emptyFlats.Id, emptyFlats.Номер_квартиры, emptyFlats.Адрес, cast(incCnt*tot.koef as int) as diff,
		cast(incCnt*tot.koef as int)*(SELECT Тариф FROM [dbo].[УслугаSet] where ХВС = 1) as val, emptyFlats.hid
	from
	...
	join
	(
		select t1.Id, (t1.Показание_ХВС - t2.Показание_ХВС - fVals.fvalDiff) / inh.inhCount as koef from 
		(	select h.Id, odu.Показание_ХВС from dbo.ДомSet h
...
exec sp_CalcHVSForPeriod @period = 14